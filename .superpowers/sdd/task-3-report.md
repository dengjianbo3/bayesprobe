...
## Re-Review Fix: Invalid Request Config Must Stay a Validation Error

### Scope
- Keep non-finite `openai_responses.timeout_seconds` on the HTTP 400 `validation_error` path.
- Preserve sanitized HTTP 502 `provider_error` handling for client factory initialization failures.
- Preserve sanitized HTTP 502 `provider_error` handling for provider execution failures.

### RED
Command:

```bash
PYTHONDONTWRITEBYTECODE=1 python3 -m pytest tests/test_webui.py -q -p no:cacheprovider -k invalid_timeout_is_validation_error
```
...
F                                                                        [100%]
=================================== FAILURES ===================================
_______ test_webui_openai_responses_invalid_timeout_is_validation_error ________

E       assert 502 == 400

=========================== short test summary info ============================
FAILED tests/test_webui.py::test_webui_openai_responses_invalid_timeout_is_validation_error
...
### Fix Notes
- Removed the broad `openai_responses` exception remap around `_build_webui_model_gateway(...)` so request-config validation failures no longer get mislabeled as provider outages.
- Converted `OpenAIModelGatewayConfig` `ValueError`s into `WebUIError` inside the request builder, preserving the HTTP 400 `validation_error` response contract.
- Narrowed `ProviderError` conversion to the `client_factory(...)` call so initialization and execution failures still return sanitized HTTP 502 `provider_error` responses.
